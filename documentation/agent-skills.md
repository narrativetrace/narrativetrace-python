...
- **Any agent, any platform**: every agent that reads `AGENTS.md` sees the always-on pointer this
  ...
  exist.
- **Codex, Gemini, and an automatic installer** are on the roadmap but not built yet — today,
  copying the rendered files is the path.
...
scripts/skills_render.py --fix` regenerates `.claude/skills/add/SKILL.md`,
`.claude/skills/doctor/SKILL.md`, and this repository's own `AGENTS.md` section from them, and
`python scripts/skills_render.py --check` (wired into `uv run poe check`) fails the build the
moment any of the three drifts from the typed source. Every code block a rendered page shows is
