...
| Per-test trace files with `.json` and Mermaid `.mmd` companions | Free | on by default *(since 0.1.2)*, `NARRATIVETRACE_OUTPUT=false` opts out; empty traces write nothing |
| AI-safe structural trace artifact (`.nt`) — value-free shape, per-invocation naming, `manifest.json`, last-green baseline with a suite-footer delta line | Free *(since 0.1.2)* | `render.structural.StructuralTraceRenderer`; see [Structural Trace Format](structural-trace-format.md) |
...
*(since 0.1.2)*, alongside the full-detail Markdown/JSON
