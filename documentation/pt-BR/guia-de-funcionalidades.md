...
| Arquivos de trace por teste com companheiros `.json` e Mermaid `.mmd` | Gratuito | ligado por padrão *(since 0.1.2)*, `NARRATIVETRACE_OUTPUT=false` desliga; traces vazios não gravam nada |
| Artefato de trace estrutural seguro para IA (`.nt`) — forma livre de valores, nomeação por invocação, `manifest.json`, baseline verde com uma linha de delta no rodapé da suíte | Gratuito *(since 0.1.2)* | `render.structural.StructuralTraceRenderer`; veja o [Formato de trace estrutural](formato-de-trace-estrutural.md) |
...
também grava um segundo arquivo *estrutural* livre de valores por invocação de teste *(since 0.1.2)*, ao lado do artefato Markdown/JSON com detalhe completo — veja o
